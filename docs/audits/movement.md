...
| ACKs de velocidad (`MoveForce*SpeedChangeAck`, turn/pitch rate y `MoveSetModMovementForceMagnitudeAck`) | Corregido en `#A06.8a/#A06.8c` a nivel parser/handler: Rust lee `MovementSpeedAck`, valida GUID/coord, registra el ACK con speed y rebroadcasts `SMSG_MOVE_UPDATE_MOD_MOVEMENT_FORCE_MAGNITUDE` para el ACK de movement-force magnitude. | OK parcial | `#A06.8b`: counters `m_forced_speed_changes`, speed runtime y anticheat/corrección/kick como C++. |
...
- `#A06.8b/#A06.8e-#A06.8g`: pendientes explícitos para speed anticheat/counters, taxi spline done, near teleport runtime y `ValidateMovementInfo` completo.
