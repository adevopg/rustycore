...
| `crates/wow-recastdetour/src/lib.rs` | `file` | 1 | partial | `portable_slice` | portable `MMapDefines.h`/Detour constants, `.mmap` params, `dtNavMesh` wrapper with tile add/remove, query/filter wrappers and `MMapManager` tile/per-instance query cache |
