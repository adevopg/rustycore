...
- First hand-rolled Detour C ABI exists for `dtNavMesh`, `dtNavMeshQuery` allocation/init/free, `dtNavMeshQuery::findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface` and `dtQueryFilter` flags/area costs; `raycast` remains pending.
...
- **`dtNavMeshQuery` methods / filter integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface` and `dtQueryFilter` flags/area costs now have wrappers and smoke coverage, but `raycast` and `PathGenerator::CreateFilter/UpdateFilter` integration are still missing.
