...
- First hand-rolled Detour C ABI exists for `dtNavMesh`, `dtNavMeshQuery` allocation/init/free and `dtQueryFilter` flags/area costs; query methods are still pending.
...
- **`dtNavMeshQuery` methods / filter integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free and `dtQueryFilter` flags/area costs now have wrappers and smoke coverage, but query methods (`findPath`, `findNearestPoly`, etc.) and `PathGenerator::CreateFilter/UpdateFilter` integration are still missing.
