...
> **Status:** ⚠️ first portable slices started — `wow-movement::PathGenerator` now has C++ constants/flags, no-navmesh shortcut fallback, geometry helpers, path length and `ShortenPathUntilDist`; `wow-recastdetour` now vendors and compiles legacy Detour C++, has first `dtNavMesh` Rust binding (`alloc/free/init/getMaxTiles/addTile/removeTile`) with real success/error smoke coverage, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, C++-like `CreateFilter/UpdateFilter` filter helpers, straight/raycast `BuildPolyPath` Detour helpers, first straight `BuildPointPath` helper and a `dtQueryFilter` wrapper, and has `MMapDefines.h` constants/flags, `MmapTileHeader`, `.mmtile` blob reader, Detour constants, `dtNavMeshParams` parsing and a C++-like `MMapManager` map cache with tile load/unload and per-instance query allocation, but no full pathgen integration yet
...
- **PathGenerator integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, `dtQueryFilter` flags/area costs, C++ filter helper rules and straight/raycast `BuildPolyPath` helper shape now have wrappers/coverage; previous-path optimization, smooth path and owner/map wiring remain open.
