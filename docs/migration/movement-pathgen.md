...
- First hand-rolled Detour C ABI exists for `dtNavMesh`, `dtNavMeshQuery` allocation/init/free, `dtNavMeshQuery::findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers and `dtQueryFilter` flags/area costs; remaining query methods are still pending.
