...
- [ ] **#MOVE-PATH.23** Wire `PathGenerator` into `MoveSplineInit::MoveTo(generatePath=true, ...)` — see [`movement-spline.md`](movement-spline.md) #MOVE-SPL.14. The creature-side bridge now accepts already-calculated Detour paths, performs the C++ `NOPATH` direct fallback, and the session tick consults the C++ mmap/ignore-pathfinding gate before entering the pathfinding branch. The world runtime now starts a dedicated mmap pathfinder worker and asks it for wander paths without storing Detour in `WorldSession`; request map/instance ids now come from the creature owner like C++ `_source`, while remaining gaps are `PhasingHandler::GetTerrainMapId` for cosmetic/child terrain maps and owner-backed filter inputs. (M)
