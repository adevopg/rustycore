...
- **PathGenerator integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, `dtQueryFilter` flags/area costs, C++ filter helper rules and first straight `BuildPolyPath/BuildPointPath` helper shape now have wrappers/coverage; previous-path optimization, raycast branch, smooth path and owner/map wiring remain open.
...
- [ ] **#MOVE-PATH.15** Port `BuildPolyPath`: first straight helper now covers `GetPolyByLocation` low/high extents, missing-poly shortcut/NOPATH, far-from-poly end clamp, same-poly and fresh `findPath` path generation. Still open: previous-path reuse/suffix optimization and raycast branch. (H)
- [ ] **#MOVE-PATH.16** Port `BuildPointPath`: straight `findStraightPath` helper path is represented, including single-point same-poly append, short/nopath fallback and force-destination branch. Still open: dispatch to `FindSmoothPath` when `_useStraightPath=false` and raycast guard branch. (H)
