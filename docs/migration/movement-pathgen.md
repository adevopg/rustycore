...
- [ ] **#MOVE-PATH.21** Port `CalculatePath(destX, destY, destZ, force_dest)` top-level entry orchestrating all the above. A Detour-level orchestrator now centralizes WoW/Detour coordinate swaps and calls `BuildPolyPath`/`BuildPointPath`; `HaveTile` is represented through real `dtNavMesh::calcTileLoc/getTileAt`; `MMapData::calculate_path_for_instance_like_cpp` now uses the per-instance query cache and returns `None` when query/tiles are unavailable; `wow-movement::PathGenerator::apply_detour_path_like_cpp` can write the result into portable path state; `WorldCreature::begin_move_spline_by_path_like_cpp` can launch those points through `MoveSplineInit::MovebyPath`. Still open: owner runtime gates (`DisableMgr`, ignore-pathfinding state, map/instance query lookup), `PathType` conversion at the runtime boundary and calling the full bridge from the game owner object. (H)
- [x] **#MOVE-PATH.22** Port `SetUseStraightPath` / `SetPathLengthLimit` / `SetUseRaycast` setters. (L)
- [ ] **#MOVE-PATH.23** Wire `PathGenerator` into `MoveSplineInit::MoveTo(generatePath=true, ...)` — see [`movement-spline.md`](movement-spline.md) #MOVE-SPL.14. The creature-side `MovebyPath` launcher exists for already-calculated paths; the full `MoveTo(generatePath=true)` owner call still needs real map/query lookup. (M)
