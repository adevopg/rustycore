...
|---|---|---:|---:|---|---|
| `crates/wow-recastdetour` | `crate_dir` | 1 | partial | `portable_slice` | Vendored legacy Detour compiles via `build.rs`; first `dtNavMesh` binding exists including `addTile/removeTile`; `MMapDefines.h` constants/flags, `MmapTileHeader`, `.mmtile` blob reader, Detour constants, `dtNavMeshParams` parser, `MMapManager` map cache/per-instance query cache and helper names/pack ids |
...
| `crates/wow-recastdetour/src/lib.rs` | `file` | 1 | partial | `portable_slice` | portable `MMapDefines.h`/Detour constants, `.mmap` params, `dtNavMesh` wrapper with tile add/remove, query/filter wrappers and `MMapManager` per-instance query cache |
...
- **`MMapManager` tile branch** — map-level cache, `.mmap` param loading, real `dtNavMesh` init and per-instance `loadMapInstance` query cache exist, but there is no operational `loadMap(mapId,x,y)` success path yet.
