...
- **PathGenerator integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, `dtQueryFilter` flags/area costs, C++ filter helper rules, straight/raycast `BuildPolyPath`, `FixupCorridor` and `GetSteerTarget` helper shape now have wrappers/coverage; previous-path optimization, full smooth path loop and owner/map wiring remain open.
...
- [ ] **#MOVE-PATH.17** Port `FindSmoothPath` + `GetSteerTarget` + `FixupCorridor` (the Detour smooth-path tutorial code, identical math). `FixupCorridor` and `GetSteerTarget` are ported/tested; remaining work is the full `FindSmoothPath` loop including `moveAlongSurface`, height adjustment and offmesh handling. (XL)
