...
- First hand-rolled Detour C ABI exists for `dtNavMesh`, `dtNavMeshQuery` allocation/init/free, `dtNavMeshQuery::findNearestPoly`, `dtNavMeshQuery::findPath`, `dtNavMeshQuery::findStraightPath` and `dtQueryFilter` flags/area costs; remaining query methods are still pending.
