...
- First hand-rolled Detour C ABI exists for `dtNavMesh`, `dtNavMeshQuery` allocation/init/free, `dtNavMeshQuery::findNearestPoly`, `dtNavMeshQuery::findPath` and `dtQueryFilter` flags/area costs; remaining query methods are still pending.
...
- **`dtNavMeshQuery` methods / filter integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath` and `dtQueryFilter` flags/area costs now have wrappers and smoke coverage, but the remaining query methods (`findStraightPath`, `moveAlongSurface`, etc.) and `PathGenerator::CreateFilter/UpdateFilter` integration are still missing.
