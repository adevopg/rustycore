...
- `crates/wow-world/src/map_manager.rs` now initializes the local Detour `MMapManager` worker with that `mapData`, matching C++ `MMapManager::InitializeThreadUnsafe(mapData)`.
...
- **Phasing terrain map resolution** — C++ `PathGenerator` calls `PhasingHandler::GetTerrainMapId(phaseShift, ownerMapId, terrain, x, y)` before `GetNavMeshQuery`. Rust can now resolve the parent/root terrain chain, but cannot faithfully implement `GetTerrainMapId` until `PhaseShift.VisibleMapIds`, `TerrainInfo::_childTerrain` and child `_gridFileExists`/`HasChildTerrainGridFile(mapId,gx,gy)` are ported.
