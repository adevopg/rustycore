...
- [ ] **#COND.22** Implement `is_source_type_valid` per source — validates that the `(SourceGroup, SourceEntry)` references something real (gossip menu exists, loot template entry exists, etc.) (H; partial: pure no-store checks for internal reference source, spell implicit target mask, area trigger source entry, object-id visibility type restrictions, loot-template existence/link target checks, quest-available source entry, NPC vendor item source entry, spell-based source entries, client-triggered area-trigger DB2 entries, terrain-swap map existence, phase area existence, startup graveyard link validation against `world_safe_locs`/`graveyard_zone`, and spawn-group template/non-system validation implemented; gossip/ObjectMgr/trainer/conversation/full area-trigger template and SpellImplicitTarget effect-shape validation remain open)
