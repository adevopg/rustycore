...
- [ ] **#COND.31** Wire `sConditionMgr` access pattern (singleton via `OnceCell` / `&'static`) into the world startup sequence so dependents can call it after `LoadConditions` (L; partial: startup loads a shared store and passes it to sessions, but no global singleton/reload-safe accessor exists yet)
