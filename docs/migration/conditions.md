...
- Source-type index builders/attachments into downstream systems (spell implicit targets) and startup/reload wiring. Gossip, phase and graveyard attachment exist as pure `wow-data` builders; loot has a runtime bridge from `LootStoreItemContext`, but all production fill callsites still need to pass the ConditionMgr-backed predicate.
