...
- `crates/wow-data/src/conditions.rs` implements the C++ `StaticConditionTypeData[]` field-usage metadata and useless-value detection helper. Rust keeps the corrected `Private Object`/`String ID` slots explicit because the inspected legacy table appears shifted around `CONDITION_PRIVATE_OBJECT`.
