...
- [ ] **#COND.22** Implement `is_source_type_valid` per source — validates that the `(SourceGroup, SourceEntry)` references something real where C++ does so; gossip menu/menu option sources intentionally pass source validation and report missing data later during attachment, matching C++ `break` in `isSourceTypeValid` plus `addToGossipMenus`/`addToGossipMenuItems` logging (H; partial: pure no-store checks for internal reference source, spell implicit target mask/effect-shape normalization, area trigger source entry, object-id visibility type restrictions, loot-template existence/link target checks, quest-available source entry, NPC vendor item + creature template source, spell-based source entries, trainer source + spell entries, conversation line template entries filtered by `ConversationLine.db2`, vehicle/spell-click creature template sources, object-id visibility creature/gameobject template sources, server-side area-trigger template keys, client-triggered area-trigger DB2 entries, terrain-swap map existence, phase area existence, startup graveyard link validation against `world_safe_locs`/`graveyard_zone`, spawn-group template/non-system validation, and gossip no-op source validation implemented; final full-case audit remains open)
