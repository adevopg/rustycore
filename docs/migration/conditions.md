...
| Mount aura display | `crates/wow-data/src/spell.rs`, `crates/wow-packet/src/packets/vehicle.rs`, `crates/wow-world/src/session.rs` | `SPELL_AURA_MOUNTED` can now use retained `SpellEffect` misc fields plus Mount/MountXDisplay condition filtering and random usable-display selection to apply/remove represented mount display and `UNIT_FLAG_MOUNT` deltas, with creature_template_model fallback and owner `SMSG_MOVE_SET_VEHICLE_REC_ID`/`SMSG_SET_VEHICLE_REC_ID` packets for represented VehicleKit create/remove; exact VehicleKit seat/accessory runtime, broadcast-to-set behavior, PetMode, and collision packet emission remain pending. |
