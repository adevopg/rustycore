...
- `#A06.8h.3e.10` connects represented creature distract/rotate to real facing-only `MoveSplineInit` launches (`MoveTo(current position)` + `SetFacing(angle)`), preserves C++'s stationary facing spline shape, applies distract final home orientation, and returns rotate `MovementInform(ROTATE,id)` as a represented payload.
