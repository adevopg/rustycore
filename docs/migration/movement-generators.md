...
- `#A06.8h.3e.10` connects represented creature distract/rotate to real facing-only `MoveSplineInit` launches (`MoveTo(current position)` + `SetFacing(angle)`), preserving C++'s stationary facing spline shape.
