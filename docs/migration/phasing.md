...
- [ ] **#PHASE.24** Implement `InitDbPhaseShift` / `InitDbPersonalOwnership` / `InitDbVisibleMapId` so creature/gameobject DB rows that carry `phaseUseFlags` / `PhaseId` / `PhaseGroup` / `terrainSwapMap` columns produce correct phase state at spawn (M; partial: C++ helper semantics implemented; visible creature spawn/respawn path now selects and applies `phaseUseFlags`/`phaseid`/`phasegroup` plus `terrainSwapMap`; remaining spawn paths still need gameobject/areatrigger/transport wiring and DB personal owner loader)
