...
- `wow-world::phasing` implements the core C++ `PhasingHandler::AddPhase` / `RemovePhase` / phase-group / visible-map mutations for a single object, including phase DB2 flag lookup, personal GUID stamping, and UI-map phase ID updates; `ControlledUnitVisitor` selection/visited rules are ported, but runtime world-object recursion is not wired yet.
...
- [ ] **#PHASE.16** Implement `PhasingHandler::on_map_change` (terrain-swap evaluation against `CONDITION_SOURCE_TYPE_TERRAIN_SWAP`, populate `VisibleMapIds` + `UiMapPhaseIds`) (H; partial: single-object `AddVisibleMapId` / `RemoveVisibleMapId` core mutation implemented)
