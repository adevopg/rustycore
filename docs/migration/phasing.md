...
- [ ] **#PHASE.24** Implement `InitDbPhaseShift` / `InitDbPersonalOwnership` / `InitDbVisibleMapId` so creature/gameobject DB rows that carry `phaseUseFlags` / `PhaseId` / `PhaseGroup` / `terrainSwapMap` columns produce correct phase state at spawn (M; partial: C++ helper semantics implemented and creature `terrainSwapMap` path now uses `InitDbVisibleMapId`; remaining spawn paths still need `phaseUseFlags`/`PhaseId`/`PhaseGroup` wiring for creature/gameobject/areatrigger)
