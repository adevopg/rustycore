...
- `wow-world::phasing` implements the core C++ `PhasingHandler::AddPhase` / `RemovePhase` / phase-group / visible-map mutations and the `OnMapChange` terrain-swap rebuild pass with an injected condition predicate; `ControlledUnitVisitor` selection/visited rules are ported, but runtime world-object recursion and canonical ConditionMgr wiring are not wired yet.
