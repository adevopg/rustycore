...
- [ ] **#VEH.6** Implement `AddVehiclePassenger`/`RemovePassenger`/`HasEmptySeat`/`GetNextEmptySeat` (M) — partial: pure seat-map helpers and C++ `RemoveAllPassengers` side-effect plan represented; live aura/script/passenger runtime side effects pending.
