...
- [ ] **#VEH.6** Implement `AddVehiclePassenger`/`RemovePassenger`/`HasEmptySeat`/`GetNextEmptySeat` (M) — partial: pure seat-map helpers, pending-aware available-seat count, C++ `AddVehiclePassenger` join scheduling plan, and C++ `RemoveAllPassengers` side-effect plan represented; live aura/script/passenger runtime side effects pending.
