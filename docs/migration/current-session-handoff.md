...
- Latest completed slice: `#NEXT.R8.ENTITIES.422 — Unit shared-vision SetWorldObject request seam` (review `APROBADO`; validation OK; committed locally at `current #422 HEAD`; no push/install/restart).
...
Overall core migration estimate after #422 Unit shared-vision `SetWorldObject` request seam: `~91.15%` (was `~91.1%` after #421; do not claim >95% until ownership/runtime gaps below are substantially closed).
