...
- Latest completed committed slice in this handoff: `#NEXT.R8.ENTITIES.407 — variable-level live Creature loaded-grid DB-backed respawn loader RNG seam`.
...
- No push/install/restart performed for #383, #384, #385, #386, #387, #388, #389, #390, #391, #392, #393, #394, #395, #396, #397, #398, #399, #400, #401, #402, #403, #404, #405, #406, or #407.
...
Overall core migration estimate after #407 variable-level live Creature loaded-grid DB-backed respawn loader RNG seam: `~89.1%` (was `~88.9%` after #406; do not claim >95%).
...
Manual test point: no new client-facing manual milestone from #407. Fixed-level and variable-level supported DB-backed Creature records can now flow through the live loaded-grid respawn seam into `MapObjectRecord`/AddToMap with C++-like inclusive level selection owned by `wow_map::Map`. This still does not support creature vehicle templates, GameObject DB-backed loaded-grid loading, full AddToWorld/ObjectAccessor/fanout/scripts/AI/zonescript/formation/dynamic-tree side effects, ObjectGridLoader rewrite, PoolMgr full live spawn work, AreaTrigger runtime, CleanupsBeforeDelete, dynamic tree/session fanout, install, restart, or push.
