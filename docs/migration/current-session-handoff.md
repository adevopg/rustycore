...
- Latest completed committed slice: `#NEXT.R8.ENTITIES.408 — GameObject loaded-grid DB-backed represented LoadFromDB/AddToMap seam for supported intrinsic rows`.
...
Overall core migration estimate after committed #408 GameObject loaded-grid DB-backed represented LoadFromDB/AddToMap seam: `~89.4%` (was `~89.1%` after #407; do not claim >95% until ownership/runtime gaps below are substantially closed).

This remains intentionally below the R8 TSV row-completion ratio because heavy runtime ownership gaps remain: vehicle runtime for creature templates with `vehicle_id` remains unsupported; GameObject loaded-grid is only represented for supported intrinsic DB rows and still lacks full AddToWorld/ObjectAccessor/fanout/scripts/AI/model/collision/linked-trap/visibility/transport runtime; real `PoolMgr` runtime execution, live `SpawnPool`/`DespawnPool` RNG/chance execution beyond deterministic planning/report, recursive subpool live integration beyond returned action records, `RemoveFromMap` side-effect completeness, corpse load, AreaTrigger Create/Load/Update runtime, templates/spawns, AI, caster unregister, unit enter/exit, movement/visibility/transport, real terrain/vmap/dynamic-tree collision, transports, visibility overrides/cinematic/sight runtime, full entity-specific `AddToWorld`/`RemoveFromWorld` side effects beyond the object/spawn-id store, real dynamic escort config/runtime feeding the closure, grid/session fanout, ObjectAccessor ownership, DB save/delete execution for runtime branches, and broader Unit/Player inventory/auras/threat/motion/update-field work.

Manual test point: no new client-facing manual milestone from #408. Fixed-level and variable-level supported DB-backed Creature records plus supported intrinsic GameObject records can flow through the live loaded-grid respawn seam into `MapObjectRecord`/AddToMap with map-owned GUID generation. This still does not support creature vehicle templates, full AddToWorld/ObjectAccessor/fanout/scripts/AI/zonescript/formation/dynamic-tree side effects, linked trap creation, model/collision, visibility distance overrides, full transport runtime, ObjectGridLoader rewrite, PoolMgr full live spawn work, AreaTrigger runtime, CleanupsBeforeDelete, dynamic tree/session fanout, install, restart, or push.
