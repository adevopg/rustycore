...
- Latest completed slice: `#NEXT.R8.ENTITIES.424 — Map-owned Player::SetViewpoint Unit-target shared-vision consumption seam` (review `APROBADO`; validation OK; committed locally at `current #424 HEAD`; no push/install/restart).
...
- No push/install/restart performed for #383 through #424.
...
Overall core migration estimate after #424 map-owned Player::SetViewpoint Unit-target shared-vision consumption seam: `~91.25%` (was `~91.15%` after #422; do not claim >95% until ownership/runtime gaps below are substantially closed).
