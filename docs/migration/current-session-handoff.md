...
- Latest completed/current slice: `#NEXT.R8.ENTITIES.427 — DynamicObject RemoveFromWorld caster-viewpoint cleanup in map removal` (review `APROBADO`; validation OK; committed locally at `current #427 HEAD`; no push/install/restart).
...
- No push/install/restart performed for #383 through #427.
...
Overall core migration estimate after #427 local DynamicObject RemoveFromWorld caster-viewpoint cleanup during map removal: `~91.55%` (was `~91.45%` after #426; do not claim >95% until ownership/runtime gaps below are substantially closed).
