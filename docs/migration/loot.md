...
- Fishing loot is represented for ready bobbers: fish/junk opens consume `LootTemplates_Fishing` with C++ parent-area fallback and default zone `1` (#NEXT.R8.ENTITIES.322). Remaining gaps are canonical bobber ownership, real area lookup, real fishing skill mutation, nearby fishing-hole search and canonical `GameObject::m_loot`.
