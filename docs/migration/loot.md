...
- [ ] **#LOOT.9** Implement canonical `Loot::generate_money_loot(min, max)` honoring `Rate.Drop.Money` config in `wow-loot` (L). Partial bridge exists only for represented `CMSG_OPEN_ITEM` item loot (#NEXT.R8.ENTITIES.190).
