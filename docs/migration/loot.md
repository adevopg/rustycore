...
- `crates/wow-world/src/session.rs` — session-local represented `GameObject::m_tapList` support added in `#NEXT.R8.ENTITIES.314` for the C++ `chestPersonalLoot + DungeonEncounter` tapper selection branch. Represented loot now marks tap-list player GUIDs as allowed looters, with the same current-player fallback C++ uses when the tap list is empty; full `LootMgr::GenerateDungeonEncounterPersonalLoot`, encounter lockouts and canonical `m_personalLoot` remain pending.
