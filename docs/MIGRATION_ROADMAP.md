...
- [ ] **#030** `wow-entities`: remaining map-stored object types: `DynamicObject`, `AreaTrigger`, `Pet`, `Transport`, `Vehicle`, `SceneObject`, `Conversation`, `Totem`; mark post-WoLK-only behavior explicitly when C++ has stubs. Refinado: base `DynamicObject` state cerrado en `#030a`, base `AreaTrigger` state cerrado en `#030b`, base `SceneObject` state cerrado en `#030c` y base `Conversation` state cerrado en `#030d`; siguen pendientes DynamicObject create/add-to-map/update runtime, AreaTrigger templates/create/load/update/search/actions/AI, SceneObject create/map/aura removal, ConversationDataStore/Start/runtime, Aura/Spell ownership, caster registration, farsight viewpoint and transport/map relocation.
