...
- [ ] **#028** `wow-entities`: `Player` from `Entities/Player/`: account/session link, inventory refs, quests, skills, taxi state. Refinado: base `Player` state/setters cerrado en `#028a`, base `Item` state cerrado en `#028b`, base `Bag` state cerrado en `#028c` y storage lookup cerrado en `#028d`; siguen pendientes create/load/login, inventario real ownership, quests, skills, taxi, social, mail, group/guild, battleground, visibility and persistence.
