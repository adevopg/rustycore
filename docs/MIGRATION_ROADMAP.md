...
- [ ] **#030** `wow-entities`: remaining map-stored object types: `DynamicObject`, `AreaTrigger`, `Pet`, `Transport`, `Vehicle`, `SceneObject`, `Conversation`, `Totem`; mark post-WoLK-only behavior explicitly when C++ has stubs. Refinado: base `DynamicObject` state cerrado en `#030a`, base `AreaTrigger` state cerrado en `#030b`, base `SceneObject` state cerrado en `#030c`, base `Conversation` state cerrado en `#030d` y base `Totem` state cerrado en `#030e`; siguen pendientes DynamicObject create/add-to-map/update runtime, AreaTrigger templates/create/load/update/search/actions/AI, SceneObject create/map/aura removal, ConversationDataStore/Start/runtime, TempSummon/Minion runtime, Aura/Spell ownership, caster registration, farsight viewpoint and transport/map relocation.
