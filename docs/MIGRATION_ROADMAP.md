...
| `Entities/Item` | wow-entities | ⚠️ | base Item+Bag+Player storage/ObjectAccessor lookup/visible item state iniciado; InventoryType bridge corregido; ownership/DB/runtime pendiente |
...
- [ ] **#028** `wow-entities`: `Player` from `Entities/Player/`: account/session link, inventory refs, quests, skills, taxi state. Refinado: base `Player` state/setters cerrado en `#028a`, base `Item` state cerrado en `#028b`, base `Bag` state cerrado en `#028c`, storage lookup cerrado en `#028d`, ObjectAccessor item branch cerrado en `#028e`, visible item state cerrado en `#028f` e InventoryType bridge cerrado en `#028g`; siguen pendientes create/load/login, inventario real ownership, template-derived item display/binding side effects, quests, skills, taxi, social, mail, group/guild, battleground and persistence.
